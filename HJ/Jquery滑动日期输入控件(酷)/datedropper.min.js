		...
		return e(this).each(function() {
			if (e(this).is("input") && "text" == e(this).attr("type")) {
				var i = (new Date).getFullYear(),
					...
					o = e(this),
					_ = e(".dd_wrap").length + 1,
					u = function(d) {
						return !(d % 4 || !(d % 100) && d % 400)
					},
					f = 100,
					c = /^#[0-9A-F]{6}$/i.test(s.color),
					v = 0;
				c || (s.color = "#f87a54"), s.maxYear < i && (i = s.maxYear);
				var m = function(d) {
						for (var e = s.minYear; e <= s.maxYear; e++) {
				...
				switch (s.lang) {
				case "ar":
					var b = ["جانفي", "فيفري", "مارس", "أفريل", "ماي", "جوان", "جويلية", "أوت", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"],
						p = ["الأحد", "الإثنين", "الثلثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
					break;
				case "it":
					var b = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"],
						p = ["Domenica", "Lunedì", "Martedì", "Mercoledì", "Giovedì", "Venerdì", "Sabato"];
					break;
				case "hu":
					var b = ["január", "február", "március", "április", "május", "június", "július", "augusztus", "szeptember", "október", "november", "december"],
						p = ["vasárnap", "hétfő", "kedd", "szerda", "csütörtök", "péntek", "szombat"];
					break;
				case "es":
					...
					break;
				case "de":
					var b = ["Januar", "Februar", "Marz", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"],
						p = ["Sonntag", "Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", "Samstag"];
					break;
				case "nl":
					var b = ["januari", "februari", "maart", "april", "mei", "juni", "juli", "augustus", "september", "oktober", "november", "december"],
						p = ["zondag", "maandag", "dinsdag", "woensdag", "donderdag", "vrijdag", "zaterdag"];
					break;
				case "fr":
					var b = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"],
						p = ["Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"];
					break;
				case "pt":
					...
					break;
				case "si":
					var b = ["januar", "februar", "marec", "april", "maj", "junij", "julij", "avgust", "september", "oktober", "november", "december"],
						p = ["nedelja", "ponedeljek", "torek", "sreda", "četrtek", "petek", "sobota"];
					break;
				default:
				...
				for (var G = 1; 12 >= G; G++) months = b[G - 1].substr(0, 3), J.find("ul").append('<li value="' + G + '">' + months + "</li>"), T.find("ul").append('<li value="' + G + '">' + G + "</li>");
				L.find(".dd_sl_").append("<ul></ul>"), R.append("<ul></ul>");
				for (var Q = 1; 31 >= Q; Q++) ddd = 10 > Q ? "0" + Q : Q, L.find("ul").append('<li value="' + Q + '">' + ddd + "<em ></em></li>"), R.find("ul").append('<li value="' + Q + '">' + ddd + "</li>");
				A.find(".dd_sl_").append("<ul></ul>");
				for (var V = s.minYear; V <= s.maxYear; V++) bissextile_return = u(V), A.find("ul").append('<li value="' + V + '" data-filter="' + bissextile_return + '">' + V + "</li>");
				if (s.years_multiple) {
					O.append("<ul></ul>"), N.append("<ul></ul>");
					for (var W = s.minYear; W <= s.maxYear; W++) {
				...
				switch (Y || S || w ? Z() : I(), "Y" != s.format && "m" != s.format && (L.find("li").click(function() {
					...
					N.find("ul").empty();
					var d = O.find("li.dd_sltd_").attr("value"),
						a = A.find("li.dd_sltd_").attr("value"),
						l = parseInt(d) + 9;
					l > s.maxYear && (l = s.maxYear), N.find("li").removeClass("dd_sltd_");
					for (var i = d; l >= i; i++) N.find("ul").append('<li value="' + i + '">' + i + "</li>");
				...
				case "Y":
					...
					break;
				case "m":
					A.hide(), O.hide(), L.hide()
				}
				var H = function() {
						var a = L.find("li.dd_sltd_").attr("value"),
							l = J.find("li.dd_sltd_").attr("value"),
							i = A.find("li.dd_sltd_").attr("value"),
							r = O.find("li.dd_sltd_"),
							t = A.find("li.dd_sltd_").attr("data-filter");
				...
				s.years_multiple && O.find("li").on("click", function() {
					...
					var d = e(this).find(".dd_sl_"),
						a = e(this).find(".dd_nav_"),
						l = d.find("li.dd_sltd_").index() * f,
						i = function() {
					...
					var d = L.find("li.dd_sltd_").attr("value"),
						e = J.find("li.dd_sltd_").attr("value"),
						a = A.find("li.dd_sltd_").attr("value");
					if (10 > d && (d = "0" + d), 10 > e && (e = "0" + e), x = new Date(e + "/" + d + "/" + a), x = x.getDay(), j = d.substr(1), D = p[x].substr(0, 3), l = p[x], n = 10 > e ? e.substr(1) : e, M = b[n - 1].substr(0, 3), F = b[n - 1], str = s.format.replace(/\b(Y)\b/i, a).replace(/\b(m)\b/i, e).replace(/\b(d)\b/i, d).replace(/\b(D)\b/i, D).replace(/\b(j)\b/i, j).replace(/\b(l)\b/i, l).replace(/\b(F)\b/i, F).replace(/\b(M)\b/i, M).replace(/\b(n)\b/i, n), s.lock) {
						...
						var o = Date.parse(d1y + "-" + d1m + "-" + d1d) / 1e3,
							_ = Date.parse(a + "-" + e + "-" + d) / 1e3;
